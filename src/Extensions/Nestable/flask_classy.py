...
class FlaskView(object):
    ...
    def make_proxy_method(cls, name):
        ...
        def proxy(**forgettable_view_args):
            ...
            # print('[%s] Return response: %s' % (i, response))
    ...
    def build_rule(cls, rule, method=None):
        ...
        if method:
            args = get_true_argspec(method).keys()
            for arg in args:
                # print('rule param %s' % arg)
...
def get_interesting_members(base_class, cls):
    ...
    return [member for member in all_members
            ...
            and inspect.signature(member[1]).parameters.get('self') is not None
            and not member[0].startswith("_")
            and not member[0].startswith("before_")
            and not member[0].startswith("after_")
            and member[0] not in cls.ignore_methods
            ]
...
def get_true_argspec(method):
    ...
    # argspec = inspect.getargspec(method)  # inspect.signature  | getargspec
    new_argspec = inspect.signature(method)
    # print(new_argspec)
    # print('========= Argspec ==========')
    # print(method)
    ...
    # print(inspect.signature(method).parameters.get('self'))
    # print('===================')
    ...
    #     return argspec
    if new_argspec.parameters and new_argspec.parameters.get('self'):
        return new_argspec.parameters
